...
class DfDownloadComp:
    ...
    def show_wb_download_button(self, wb: Workbook) -> None:
        ...
        self._DL_button_as_xlsx(wb_bytes_io)
    ...
    def _set_download_button_as_xlsx(self, bytes_io: BytesIO) -> None:
        ...
        st.download_button(
            ...
            data=data,
            download_file_name=f"{self.download_file_name}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )

    def show_df_download_button_as_csv(self, df: pd.DataFrame, encoding="utf-8") -> None:
        csv_data = df.to_csv(index=False).encode(encoding)
        st.download_button(
            label="Download data as CSV",
            data=csv_data,
            download_file_name=f"{self.download_file_name}.csv",
            mime="text/csv",
        )
